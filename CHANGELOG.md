...
- Configuración de exclusión en `pyproject.toml` para ignorar los directorios `gui/` y `cli/` temporalmente en reportes de cobertura.
...
- `NumpyEncoder`: Clase JSON encoder personalizada
  - Convierte np.ndarray, np.integer, np.floating, np.bool\_
